<?php
/**
 * crm/customers/edit.php
 *
 * Edit an existing customer record.
 * Mirrors the create form but pre-populates fields and handles UPDATE.
 */
...
$stmt = $db->prepare("SELECT * FROM crm_customers WHERE id = :id");
$stmt->execute([':id' => $id]);
$customer = $stmt->fetch();
...
// Pre-populate form data from the existing record.
$formData = $customer;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!crmValidateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($errors)) {
            ...
            // Replace tags with the new set.
            $tagString = trim($_POST['tags'] ?? '');
            crmSetTags('customer', $id, $tagString !== '' ? array_map('trim', explode(',', $tagString)) : []);

            crmLogActivity('updated', 'customer', $id,
                $formData['first_name'] . ' ' . $formData['last_name']);

            crmFlash('Customer updated.', 'success');
            crmRedirect(SITE_URL . '/crm/customers/view.php?id=' . $id);
        }
    }
}

// Current tags as comma string for the input field.
$currentTags  = crmGetTags('customer', $id);
$currentTagStr = implode(', ', array_column($currentTags, 'name'));
...
<div class="admin-header">
    <div>
        <h1>&#128100; Edit Customer</h1>
        <p>
            <a href="<?= SITE_URL ?>/crm/customers/view.php?id=<?= $id ?>">&#8592; Back to Profile</a>
        </p>
    </div>
</div>
    ...
    <form method="post" action="">
        ...
        <div class="form-group">
            ...
            <input type="text" id="tags" name="tags" class="form-control"
                   value="<?= htmlspecialchars($currentTagStr, ENT_QUOTES) ?>">
        </div>

        <div class="form-actions">
            <button type="submit" class="btn btn--primary">Save Changes</button>
            <a href="<?= SITE_URL ?>/crm/customers/view.php?id=<?= $id ?>" class="btn btn--outline">Cancel</a>
            <?php if (crmCanDelete()): ?>
                <a href="<?= SITE_URL ?>/crm/customers/delete.php?id=<?= $id ?>&csrf=<?= crmCsrfToken() ?>"
